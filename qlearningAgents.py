...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        for action in legal_action:
            best_action[action] = self.getQValue(state, action)
    ...
    def update(self, state, action, nextState, reward):
        ...
        utility = self.getValue(nextState)
        max_util = q_value + self.alpha * (reward + (self.discount * utility) - q_value)
        self.q_value[(state, action)] = max_util
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        return feature_vector * self.weights
    ...
    def update(self, state, action, nextState, reward):
        ...
        utility = self.getValue(nextState)
        feature_vector = self.featExtractor.getFeatures(state, action)
        delta = (reward + self.discount * utility) - q_value

        for feature in feature_vector:
            weight = self.alpha * delta * feature_vector[feature]
